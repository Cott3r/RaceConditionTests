...
pthread_cond_t buffer_elements_free;
pthread_cond_t buffer_elements_available;
int buffer_number_elements_free = 16;
int buffer[16] = { 0 };
...
void* producer(void* p)
{
  ...
  while (numbers_produced < TOTAL)
  {
    ...
    while (success == 0)
    {
      ...
      if (buffer_number_elements_free == 0)
      {
        pthread_cond_wait(&buffer_elements_free,&buffer_lock);
      }
      buffer_number_elements_free--;
      for (size_t i = 0; i < 16; ++i)
      {
        if (buffer[i] == 0)
        {
          buffer[i] = r;
          pthread_cond_signal(&buffer_elements_available);
          success = 1;
          break;
        }
      }
...
void* consumer(void* p)
{
  ...
  while (numbers_consumed < TOTAL)
  {
    ...
    {
      pthread_mutex_lock(&buffer_lock);
      if (buffer_number_elements_free == 16)
      {
        pthread_cond_wait(&buffer_elements_available,&buffer_lock);
      }
      for (size_t i = 0; i < 16; ++i)
      {
        if (buffer[i] != 0)
        {
          ...
          buffer[i] = 0;
          buffer_number_elements_free++;
          pthread_cond_signal(&buffer_elements_free);
          success = 1;
          break;
        }
      }
...
int main()
{
//  pthread_cond_init(&buffer_elements_free,0);
//  pthread_cond_init(&buffer_elements_available,0);
